...
class BinMapper {
  ...
  */
  ...
  inline double min_val()const{ return min_val_;}

  inline double max_val()const{ return max_val_;}

  inline std::unordered_map<int, unsigned int> categorical_2_bin()const{ return categorical_2_bin_;}

  inline std::vector<int> bin_2_categorical()const{ return bin_2_categorical_;}
 ...
 public:
  const std::vector<double> GetUpperBoundValue() const { return bin_upper_bound_; }
  // just for pickle 
  BinMapper(int num_bin, MissingType missing_type, const std::vector<double> bin_upper_bound, bool is_trivial, double sparse_rate,
            BinType bin_type, const std::unordered_map<int, unsigned int> &categorical_2_bin, 
            const std::vector<int> &bin_2_categorical, double max_val, double min_val, uint32_t default_bin, 
            uint32_t most_freq_bin) : num_bin_(num_bin), missing_type_(missing_type), is_trivial_(is_trivial), 
            sparse_rate_(sparse_rate), bin_type_(bin_type), max_val_(max_val), min_val_(min_val), 
            default_bin_(default_bin), most_freq_bin_(most_freq_bin), categorical_2_bin_(categorical_2_bin), 
            bin_2_categorical_(bin_2_categorical), bin_upper_bound_(bin_upper_bound){}
};
