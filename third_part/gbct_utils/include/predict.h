...
int predict_single(const std::vector<T>& tree,
...
template<typename T, typename = std::enable_if_t<std::is_base_of<Node, T>::value>>
void predict_single(const std::vector<std::vector<T>>& trees,
                    py::array_t<double> out,
                    const py::array_t<double>& x,
                    const std::string& key) {
    if (c_style(out) == false) {
        throw std::runtime_error("fortune style `out` is not support!");
    }
...
void predict_batch(const std::vector<std::vector<T>>& trees,
  ...
  if (c_style(x) == false) {
    throw std::runtime_error("fortune style `x` is not support!");
  }
    int n = x.shape(0);
