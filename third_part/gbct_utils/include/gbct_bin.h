...
std::vector<LightGBM::BinMapper> FindBinParallel(
    const py::array_t<dtype>& data,
    int max_bin = 64,
    int min_data_in_bin = 100,
    int min_split_data = 100,
    bool pre_filter = false,
    int bin_type = LightGBM::BinType::NumericalBin,
...
#pragma omp parallel for
    for (size_t i = 0; i < nfeat; i++) {
        for (size_t j = 0; j < ninst; j++) {
            if (data.at(j, i) != dtype(0))
                nzero_data[i].push_back(data.at(j, i));
        }
    }

#pragma omp parallel for
    for (size_t i = 0; i < nfeat; i++) {
        mappers[i].FindBin(nzero_data[i].data(), nzero_data[i].size(), ninst,
                           max_bin, min_data_in_bin, min_split_data, pre_filter,
                           LightGBM::BinType(bin_type), use_missing,
...
py::array_t<idtype>& Value2BinParallel(
    const py::array_t<dtype>& data,
    const std::vector<LightGBM::BinMapper>& mappers,
    py::array_t<idtype>& out,
    int num_threads = -1) {
    size_t nfeat = mappers.size();
    size_t ninst = data.shape(0);
    size_t block_size = 10240;
    if (nfeat != data.shape(1))
        LightGBM::Log::Fatal("data(%d) and mappers(%d) don't match", data.shape(0), nfeat);
...
#pragma omp parallel for
    for (size_t i = 0; i < nfeat; i++) {
        for (size_t j = 0; j < ninst; j++) {
            out.mutable_at(j, i) = mappers[i].ValueToBin(data.at(j, i));
        }
    }
